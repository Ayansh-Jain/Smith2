  ...
  return (
    <main style={{ minHeight: '100vh', background: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '120px 20px 60px' }}>
      <motion.div
        ...
        style={{ background: '#fff', borderRadius: '28px', padding: '56px 48px', textAlign: 'center', maxWidth: '520px', width: '100%', boxShadow: '0 30px 80px rgba(0,0,0,0.4)' }}
      >
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: 'spring', stiffness: 200 }}>
          <FaTimesCircle style={{ fontSize: '5rem', color: '#EF4444', marginBottom: '20px' }} />
        </motion.div>
        <h1 style={{ fontSize: '2.2rem', fontWeight: 900, color: '#111', marginBottom: '12px' }}>Payment Failed</h1>
        <p style={{ color: '#666', fontSize: '1rem', marginBottom: '8px', lineHeight: 1.7 }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <motion.div whileHover={{ scale: 1.04 }}>
            <Link to="/booking" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 28px', background: 'linear-gradient(135deg, #D4AF37, #FFD700)', color: '#fff', borderRadius: '14px', fontWeight: 700, textDecoration: 'none' }}>
              <FaRedo /> Try Again
            </Link>
          </motion.div>
          <motion.div whileHover={{ scale: 1.04 }}>
